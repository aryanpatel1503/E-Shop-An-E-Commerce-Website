...
const Sidebar = () => {
  ...
  const isActive = (path) => {
    if (
      pathname === `/admin/add${path}` ||
      pathname.startsWith(`/admin/edit${path}`)
    ) {
      return "!bg-[#dbeafe] !text-[#3b82f6] !border-r-[3px] !border-[#3b82f6]";
    }
  };
  ...
  return (
    <aside
      id="logo-sidebar"
      className="fixed top-0 left-0 z-40 uppercase h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700 w-auto md:w-3/12 lg:w-2/12"
      aria-label="Sidebar"
    >
      <div className="h-full pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        <ul className="space-y-1 font-medium h-full flex flex-col w-full">
          <li>
            <NavLink
              to="/admin/dashboard"
              style={active}
          ...
          <li>
            <NavLink
              to="/admin/viewuser"
              style={active}
              className={`flex items-center pl-5 py-3 text-gray-900 hover:border-r-[3px] hover:border-blue-500 hover:text-blue-500 hover:bg-blue-100 dark:text-white dark:hover:bg-gray-700 ${isActive(
                "customer"
              )}`}
            >
              <svg
